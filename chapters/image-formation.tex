...
Real images do not represent the true sky signal exactly.
Numerous effects alter and degrade the true sky signal.
Some of these effects are intrinsic to the nature of light, and while processing can reduce their impact on an image, they cannot be avoided.
Other effects are external, and depend on the specific nature of the optical system used to generate the image.
...
Because light has a wave-like nature, it is also physically impossible to focus light to a single point.
In terms of angles in the sky, the minimum spot size is limited by the aperture of the optical system.
At the sensor, the focal ratio determines the physical scale of the spot, with higher focal ratios corresponding to larger spot sizes.
\\[\baselineskip]
The \textit{Airy disk} describes the spot generated by a given aperture.
The profile of the Airy disk for an obstructed circular aperture is given by the function\footnote{Not to be confused with the \textit{Airy functions}, $\operatorname{Ai}\left(x\right)$ and $\operatorname{Bi}\left(x\right)$, which arise as solutions to the Airy--Stokes differential equation, $\frac{d^2 y}{dx^2} - xy = 0$.}
\begin{flalign}
\operatorname{Airy}\left(x\right) & =
    I_{0} \left(\frac{2 J_{1}\left(x\right) - 2 \epsilon J_{1}\left(\epsilon x\right)}{x \left(1 - \epsilon^{2}\right)}\right)^{2}
\end{flalign}
where $I_{0}$ is the peak irradiance, $J_{1}$ is the first order Bessel function of the first kind, and $\epsilon$ is the linear obstruction ratio, which is zero for unobstructed apertures.
The quantity $x$ can be calculated from the diameter of the aperture, $D$, the wavelength of light, $\lambda$, and the angle at which we are measuring the intensity, $\theta$.
\begin{flalign}
x & =
    \frac{\pi D \sin \left(\theta\right)}{\lambda}
\end{flalign}
This is useful for opticians and visual astronomers who are most interested in determining how the Airy disk limits their ability to resolve bright objects, like double stars.
\\[\baselineskip]
Imagers are often more interested in the physical size of the Airy disk on the image plane.
For this purpose, we can redefine $x$ in terms of the focal ratio $N$ and the focal length $f$:
\begin{flalign}
x & =
    \frac{\pi f \sin \left(\theta\right)}{N \lambda}
\end{flalign}
Now we define $r$ as the physical distance from the center of the Airy disk at which we are evaluating the function.
If the image plane is flat, $f$ and $r$ form the sides of a right triangle, so the term $\sin \left(\theta\right)$ corresponds to the ratio of the lengths of the side opposite $f$ and the hypotenuse:
\begin{flalign}
x & =
    \frac{\pi f r}{N \lambda \sqrt{f^2 + r^2}}
\end{flalign}
For the vast majority of cases, we can use a small angle approximation, since $\theta$ will rarely exceed more than a few arcseconds. This allows us to eliminate $f$ from the equation by pulling out the ratio between $f$ and the hypotenuse, equal to $\cos \left(\theta\right)$:
\begin{flalign}
x & =
    \frac{\pi r \cos \left(\theta\right)}{N \lambda} \\
& \approx
    \frac{\pi r}{N \lambda}
\end{flalign}
With this in mind, we can express the Airy disk in terms of the imaging parameters:
\begin{flalign}
\operatorname{Airy}\left(\lambda, f, N, r\right) & =
    I_{0} \left(\frac{2 J_{1}\left(\frac{\pi f r}{N \lambda \sqrt{f^2 + r^2}}\right) - 2 \epsilon J_{1}\left(\epsilon \frac{\pi f r}{N \lambda \sqrt{f^2 + r^2}}\right)}{\frac{\pi f r}{N \lambda \sqrt{f^2 + r^2}} \left(1 - \epsilon^{2}\right)}\right)^{2}
\end{flalign}
One further simplification can be made by defining the \textit{sombrero function}, also called a \textit{besinc} or \textit{jinc function}\footnote{The factors of $2$ or $\pi$ are omitted from the definition in some literature.}:
\begin{flalign}
\operatorname{somb}\left(x\right) = \frac{2 J_{1} \left(\pi x\right)}{\pi x}
\end{flalign}
This allows us to shorten our defintion:
\begin{flalign}
\operatorname{Airy}\left(\lambda, f, N, r\right) & =
    I_{0} \left(
        \frac{\operatorname{somb}\left(\frac{f r}{N \lambda \sqrt{f^2 + r^2}}\right) - \epsilon \operatorname{somb}\left(\frac{f r}{N \lambda \sqrt{f^2 + r^2}}\right)}{1 - \epsilon^{2}}
    \right)^{2} \\
& \approx
    I_{0} \left(
        \frac{\operatorname{somb}\left(\frac{r}{N \lambda}\right) - \epsilon \operatorname{somb}\left(\frac{r}{N \lambda}\right)}{1 - \epsilon^{2}}
    \right)^{2}
\end{flalign}
Many software libraries that implement special functions specifically implement this function efficiently.
\\[\baselineskip]
As we have defined it, the Airy disk function describes the diffraction of light at exactly one wavelength.
This approximation is sufficient for narrowband filters, but for broadband filters, we need to make an adjustment to get a correct Airy profile.
To determine the profile shape for a given filter, we can integrate over the transmission profile of the imaging system.
For the common imaging systems we use as astronomers, we can break the transmission down into two factors: the quantum efficiency of the camera and the filter profile.
These only need to be relative transmission values, since the magnitude of the Airy disk brightness depends on the brightness of the source.
\begin{flalign}
T\left(\lambda\right) & = 
    T_\text{filter}\left(\lambda\right) \operatorname{QE}\left(\lambda\right)
\end{flalign}
This term can then be used to weight an integral over the Airy disk expected at each wavelength, taken over a reasonable choice of minimum and maximum wavelengths $\lambda_\text{min}$ and $\lambda_\text{max}$:
\begin{flalign}
\operatorname{Airy}\left(T, \lambda, f, N, r\right) & = 
    \int_{\lambda_\text{min}}^{\lambda_\text{max}}
    I_{0} \left(
        \frac{\operatorname{somb}\left(\frac{f r}{N \lambda \sqrt{f^2 + r^2}}\right) - \epsilon \operatorname{somb}\left(\frac{f r}{N \lambda \sqrt{f^2 + r^2}}\right)}{1 - \epsilon^{2}}
    \right)^{2}
    T\left(\lambda\right) d \lambda \\
& \approx
    \int_{\lambda_\text{min}}^{\lambda_\text{max}}
    I_{0} \left(
        \frac{\operatorname{somb}\left(\frac{r}{N \lambda}\right) - \epsilon \operatorname{somb}\left(\frac{r}{N \lambda}\right)}{1 - \epsilon^{2}}
